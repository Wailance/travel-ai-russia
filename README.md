...
- Параметры: срок, города старта/финиша, **количество городов**, бюджет, проживание, своё авто.
- **Цели поездки** (до 3): гастрономия, православие, культура, достопримечательности и др.
- Генерация маршрута через GigaChat API.
- Результат: путь, расписание по дням, логистика, бюджет, графики, карта (Leaflet).
...
3. Вставьте ключ авторизации в `GIGACHAT_AUTH_KEY` (см. `.env.example`).
...
| Сервис | URL |
|--------|-----|
| Фронт (GitHub Pages) | https://wailance.github.io/travel-ai-russia/ |
| API (Render) | https://travel-ai-russia-api.onrender.com |

`public/config.js` на GitHub Pages автоматически указывает на Render API.
