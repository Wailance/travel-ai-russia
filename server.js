...
const GENERIC_PLACE_PATTERNS = [
  /^(место|локация|достопримечательност|экскурсия|музей|ресторан|кафе|парк|активност|прогулка|обед|ужин|завтрак|питание|проживание|отель|гостиница|транспорт|смотровая|площадь|набережная|центр города|магазин|рынок|бар|клуб|галерея|сувенир)/i,
  /^(посещение|осмотр|знакомство|экскурсия по|обзор|свободное время|отдых|развлечени|прогулка по|пешеходная|обзорная|вечерняя|утренняя)/i,
  /^(активности и экскурсии|транспорт по маршруту|местная кухня|национальная кухня|традиционная кухня|местный ресторан|городской парк)$/i,
  /лучший|популярн|известн|необычн|уникальн|романтичн|уютн|красив|знаменит|секретн|малоизвестн|легендарн|топ-\d|модн|трендов|аутентичн|премиальн/i,
  /^(кафе|ресторан|бар|паб|бистро|столовая|кофейня|пекарня|пиццерия|суши|шашлычная)\s+(на|у|в|около|возле|рядом)\s/i
];
...
function isGenericPlaceName(place) {
  ...
  if (/^(кафе|ресторан|музей|парк|отель|площадь|собор|храм|театр|еда|питание|проживание|бар|клуб|галерея|рынок|магазин|базар|торговый центр|ТЦ|фудкорт|столовая|кофейня|пекарня)$/i.test(p)) {
    return true;
  }
  if (/^(кафе|ресторан|бар|музей|парк|отель)\s+["«]?.{0,3}["»]?$/i.test(p)) return true;
  return false;
}

function looksPossiblyHallucinated(place) {
  return /(лучший|топ-|секретн|малоизвестн|легендарн|уникальн|незабываем|скрытый|must.?see|instagram|аутентичн|эксклюзивн|премиальн|авторск|модн|трендов|hidden.?gem|рекомендуем|любимый|особенный|волшебн|сказочн|изысканн|великолепн|восхитительн|потрясающ)/i.test(
    String(place || "")
  );
}
...
const CITY_RECOMMENDATIONS = {
  "москва": {
    food: ["Депо Москва (фудмолл)", "Вареничная №1 (Арбат)", "ДжонДжоли (Смоленская)", "Теремок (Тверская)", "Му-Му (Арбат)", "Чайхона №1 (Тверской)", "Кофемания (Лубянка)", "Братья Караваевы (Патриаршие)", "Lavash (Тверская)"],
    hotel: ["Azimut Сити Отель Смоленская", "Ibis Moscow Centre Bakhrushina", "Holiday Inn Moscow Sokolniki", "Космос", "Гостиница Украина", "Метрополь"],
    activity: ["Красная площадь", "Государственный исторический музей", "ВДНХ", "Парк Зарядье", "Третьяковская галерея", "Московский Кремль", "Храм Христа Спасителя", "Большой театр", "Музей космонавтики", "Парк Горького", "Новодевичий монастырь", "Арбат", "Воробьёвы горы", "ГУМ", "Пушкинский музей"]
  },
  "санкт-петербург": {
    food: ["Marketplace (Невский проспект)", "Pkhali Khinkali (Садовая)", "Брынза (Невский)", "Теремок (Невский)", "Пироговый дворик", "Столовая №1 (Миллионная)", "Кафе Зингеръ (Дом книги)", "Чайная ложка"],
    hotel: ["AZIMUT Сити Отель Санкт-Петербург", "Ibis Saint Petersburg Centre", "Station Hotel M19", "Гостиница Астория", "Гранд Отель Европа"],
    activity: ["Эрмитаж", "Дворцовая площадь", "Исаакиевский собор", "Петропавловская крепость", "Невский проспект", "Русский музей", "Казанский собор", "Спас на Крови", "Кунсткамера", "Петергоф", "Мариинский театр", "Летний сад", "Стрелка Васильевского острова", "Крейсер Аврора"]
  },
  "владимир": {
    food: ["Ресторан Обломов", "Кафе Гости", "Bulochnaya #1", "Мятная Сова", "Теремок"],
    hotel: ["AMAKS Золотое Кольцо", "Вознесенская Слобода", "Гостиница Владимир"],
    activity: ["Золотые ворота", "Успенский собор", "Парк Победы", "Водонапорная башня (смотровая)", "Дмитриевский собор", "Патриарший сад", "Музей Старый Владимир"]
  },
  "ярославль": {
    food: ["Пенаты", "Собрание", "Кафе АндерСон", "Иоанн Васильевич", "Буфет"],
    hotel: ["Ring Premier Hotel", "Ibis Yaroslavl Center", "Cosmos Yaroslavl Hotel"],
    activity: ["Стрелка Волги и Которосли", "Успенский собор", "Спасо-Преображенский монастырь", "Волжская набережная", "Ярославский музей-заповедник", "Церковь Ильи Пророка", "Губернаторский сад"]
  },
  "сергиев посад": {
    food: ["Русский Дворик", "Гостевая Изба", "Келарская Набережная", "Трапезная"],
    hotel: ["Царская Деревня", "Посадский", "Барские Полати"],
    activity: ["Троице-Сергиева лавра", "Конный двор", "Музей игрушки", "Пруд Красные баньки", "Черниговский скит"]
  },
  "суздаль": {
    food: ["Трапезная Троице-Сергиевой лавры (Суздаль)", "Гостиный двор", "Кафе Старый город", "Чайная", "Ресторан Улей"],
    hotel: ["Суздаль", "Пушкарская слобода", "Отель Суздаль", "Николаевский Посад"],
    activity: ["Кремль Суздаля", "Спасо-Евфимиев монастырь", "Музей деревянного зодчества", "Торговые ряды", "Покровский монастырь", "Ризоположенский монастырь"]
  },
  "ростов великий": {
    food: ["Русь", "Погост", "У Погоста", "Алёша Попович"],
    hotel: ["Сосновый бор", "Ростов", "Спасо-Яковлевский монастырь (гостиница)"],
    activity: ["Ростовский кремль", "Спасо-Яковлевский монастырь", "Озеро Неро", "Музей финифти", "Успенский собор"]
  },
  "нижний новгород": {
    food: ["Вареничная №1", "Безухов", "Волга", "Тесто", "Шоколад", "Молоко"],
    hotel: ["Azimut Отель Нижний Новгород", "Ibis Nizhny Novgorod", "Sheraton Nizhny Novgorod"],
    activity: ["Нижегородский кремль", "Чкаловская лестница", "Стрелка Оки и Волги", "Большая Покровская улица", "Нижегородская канатная дорога", "Музей-квартира Горького", "Нижегородская ярмарка"]
  },
  "казань": {
    food: ["Тубэтей", "Дом татарской кулинарии", "Бульвар на Баумана", "Чак-чак", "Добрая столовая"],
    hotel: ["Korston Club Hotel", "Ibis Kazan", "Courtyard by Marriott Kazan", "Мираж"],
    activity: ["Казанский Кремль", "Храм всех религий", "Улица Баумана", "Национальный музей Республики Татарстан", "Мечеть Кул-Шариф", "Башня Сююмбике", "Центр семьи Казан", "Кремлёвская набережная"]
  },
  "екатеринбург": {
    food: ["Паштет", "Папа Карло", "Колбасофф", "Пельменная Дюжина", "Хмели Сунели"],
    hotel: ["Hyatt Regency Ekaterinburg", "Ibis Ekaterinburg Center", "Novotel Yekaterinburg"],
    activity: ["Плотинка", "Храм на Крови", "Екатеринбургский музей изобразительных искусств", "Высоцкий смотровая", "Музей истории Екатеринбурга", "Ельцин Центр", "Литературный квартал"]
  },
  "сочи": {
    food: ["Гагри", "Сан-Ремо", "Хинкальная на Навагинской", "Белые ночи", "Чайхона Базар"],
    hotel: ["Radisson Collection Paradise", "Hyatt Regency Sochi", "Bridge Resort", "Pullman Сочи"],
    activity: ["Сочинский национальный парк", "Олимпийский парк", "Дендрарий", "Красная Поляна (канатная дорога)", "Парк Ривьера", "Навалищенское ущелье", "Агурские водопады", "Тисо-самшитовая роща", "Сочинский художественный музей"]
  },
  "новосибирск": {
    food: ["Beerman & Grill", "Тюбетей", "Пепперони", "Суши-Мастер", "La Maison"],
    hotel: ["Novotel Novosibirsk", "DoubleTree by Hilton", "Azimut Отель Новосибирск"],
    activity: ["Новосибирский государственный художественный музей", "Театр оперы и балета", "Метромост", "Зоопарк", "Новосибирский краеведческий музей", "Монумент Славы"]
  },
  "переславль-залесский": {
    food: ["Хлебник", "Трапезная", "Русский дворик", "Берендеевка"],
    hotel: ["Русь", "Переславль", "Хостел на Красной площади"],
    activity: ["Красная площадь", "Спасо-Преображенский собор", "Музей утюга", "Плещеево озеро", "Синий камень", "Никитский монастырь", "Горицкий монастырь"]
  },
  "тверь": {
    food: ["Теремок", "Позиция", "Застолье"],
    hotel: ["Гранд Отель Тверь", "Тверь Парк Отель"],
    activity: ["Тверской кремль", "Путевой дворец", "Набережная Степана Разина", "Старый мост", "Музей тверского быта"]
  },
  "калининград": {
    food: ["Хмель", "Фиш Клуб", "Мадам Буше"],
    hotel: ["Radisson Hotel Kaliningrad", "Ibis Kaliningrad Center"],
    activity: ["Кафедральный собор", "Музей Мирового океана", "Остров Канта", "Рыбная деревня", "Бранденбургские ворота", "Куршская коса"]
  },
  "волгоград": {
    food: ["Маруся", "Волгоград Плаза (ресторан)", "Пиццерия Лаваш"],
    hotel: ["Гостиница Волгоград", "Hampton by Hilton Volgograd"],
    activity: ["Мамаев курган", "Родина-мать зовёт", "Музей-панорама Сталинградская битва", "Набережная 62-й Армии"]
  }
};
...
async function verifyPlaceViaWikipedia(placeName, city, cache) {
  const cacheKey = `wiki-verify||${placeName}||${city}`.toLowerCase();
  if (cache.has(cacheKey)) return cache.get(cacheKey);

  const queries = [placeName, `${placeName} ${city}`];
  for (const query of queries) {
    try {
      ...
      const placeNorm = placeName.toLowerCase().replace(/[«»""()]/g, "").trim();
      const found = results.some((r) => {
        const title = (r.title || "").toLowerCase();
        const snippet = (r.snippet || "").toLowerCase().replace(/<[^>]*>/g, "");
        return (
          title.includes(placeNorm.slice(0, 12)) ||
          placeNorm.includes(title.slice(0, 12)) ||
          snippet.includes(placeNorm.slice(0, 12))
        );
      });
      if (found) {
...
async function groundPlanPlaces(plan, { maxGeocodes = 40 } = {}) {
  ...
  for (let dayIndex = 0; dayIndex < plan.days.length; dayIndex += 1) {
    ...
    for (let itemIndex = 0; itemIndex < day.items.length; itemIndex += 1) {
      ...
      if (isKnownVenue(place, city)) {
        item.verified = true;
        verified += 1;
        continue;
      }

      if (verifyChecks >= maxGeocodes) continue;

      if (looksPossiblyHallucinated(place)) {
        const previous = place;
        item.place = pickSpecific(city, type, dayIndex + itemIndex);
        item.comment = `${item.comment || ""} Заменено: «${previous}» — подозрение на несуществующее место.`.trim();
        item.cost = null;
        item.priceStatus = "unknown";
        item.grounded = true;
        item.verified = true;
      ...
      verifyChecks += 1;
      const geoResult = await geocodePlace(place, city, cache);
      const wikiResult = await verifyPlaceViaWikipedia(place, city, cache);

      if (geoResult && wikiResult) {
        item.verified = true;
        verified += 1;
      } else if (geoResult || wikiResult) {
        item.verified = true;
        verified += 1;
      } else {
        const previous = place;
        item.place = pickSpecific(city, type, dayIndex + itemIndex);
        item.comment = `${item.comment || ""} Заменено: «${previous}» не найдено ни на карте, ни в энциклопедии.`.trim();
        item.cost = null;
        item.priceStatus = "unknown";
        item.grounded = true;
        item.verified = true;
  ...
  return { plan, stats: { replaced, verifyChecks, verified } };
}
